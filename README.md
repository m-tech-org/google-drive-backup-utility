...
1. Download the package for your OS. Each platform is published on its own branch of this repo, not on `master`:
   - **Linux x86_64:** [gdrive-backup-utility.tar.gz](https://github.com/m-tech-org/google-drive-backup-utility/raw/linux/gdrive-backup-utility.tar.gz)
   - **Linux ARM64:** [gdrive-backup-utility-linux-arm64.tar.gz](https://github.com/m-tech-org/google-drive-backup-utility/raw/linux/gdrive-backup-utility-linux-arm64.tar.gz)
   - **Windows x86_64:** [gdrive-backup-utility-windows.zip](https://github.com/m-tech-org/google-drive-backup-utility/raw/windows/gdrive-backup-utility-windows.zip) (extract and run `gdrive-backup-utility.exe`)
   - **macOS (Apple Silicon):** [gdrive-backup-utility-macos.tar.gz](https://github.com/m-tech-org/google-drive-backup-utility/raw/macos/gdrive-backup-utility-macos.tar.gz)
...
1. Extract as above (*cloned this repo? `make setup` auto-detects your OS/arch, downloads the matching package from its branch, and extracts it; later `make update` re-downloads and upgrades in place, keeping your `.env` and credentials*).
...
**Windows/macOS?**  Yes — packages are built for Linux x86-64/ARM64, Windows x86-64, and macOS (Apple Silicon), each published to its own branch of this repo (`linux`, `windows`, `macos` — see Quick start above for direct links). There are no GitHub Releases; branches are the distribution channel. Windows packages include a Scheduled-Task service installer (`install-service.ps1`, run as administrator — Windows shows its own approval prompt), macOS packages a launchd agent (`install-service.sh`, no admin needed). The UI works on all three.
